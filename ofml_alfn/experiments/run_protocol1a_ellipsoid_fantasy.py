...
from ofml_alfn.benchmarks.dataset_builders.build_problem_1a_ellipsoid import (
...
from ofml_alfn.utils.protocol_types import BenchmarkSample, ProtocolObservation
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Problem 1A Ellipsoid experiments with mode switching."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Common padded output dimension used by current runners.",
    )

    parser.add_argument("--chart_radius", type=float, default=0.90)
    parser.add_argument(
        "--ellipsoid_center",
        type=float,
        nargs=4,
    ...
    parser.add_argument(
        ...
        type=float,
        nargs=4,
        default=[1.0, 1.2, 0.8, 1.5],
        metavar=("D1", "D2", "D3", "D4"),
    )
    parser.add_argument("--chart_sign", type=int, default=1, choices=[-1, 1])
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/protocol1a_ellipsoid")
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.input_dim != 3:
        raise ValueError(f"Ellipsoid runner expects --input_dim 3, got {args.input_dim}")
    if args.latent_dim != 4:
        raise ValueError(f"Ellipsoid runner expects --latent_dim 4, got {args.latent_dim}")
    if args.common_output_dim < 3:
        raise ValueError(
            f"Ellipsoid runner expects --common_output_dim >= 3, got {args.common_output_dim}"
        )
    ...
    if len(args.protocol_costs) != 3:
        raise ValueError("--protocol_costs must have length 3")
    if len(args.similarities_to_target) != 3:
    ...
    if len(args.ellipsoid_center) != 4:
        raise ValueError("--ellipsoid_center must have length 4")
    if len(args.ellipsoid_scales) != 4:
        raise ValueError("--ellipsoid_scales must have length 4")

    if float(args.similarities_to_target[2]) != 1.0:
        raise ValueError("similarities_to_target[2] must be 1.0 for target protocol 3")

    for i, scale in enumerate(args.observer_scales):
        if float(scale) <= 0.0:
            raise ValueError(f"observer_scales[{i}] must be positive, got {scale}")

    if float(args.target_noise_std) < 0.0:
        raise ValueError(f"--target_noise_std must be non-negative, got {args.target_noise_std}")
    for i, noise_std in enumerate(args.source_noise_stds):
        if float(noise_std) < 0.0:
            raise ValueError(f"source_noise_stds[{i}] must be non-negative, got {noise_std}")
...
def _pad_tensor_last_dim(x: torch.Tensor, target_dim: int) -> torch.Tensor:
    if x.ndim == 0:
        x = x.reshape(1)
    if x.ndim != 1:
        raise ValueError(f"Expected 1D tensor for padding, got shape {tuple(x.shape)}")
    if x.shape[0] > target_dim:
        raise ValueError(
            f"Cannot pad tensor of dim {x.shape[0]} to smaller target_dim={target_dim}"
        )
    if x.shape[0] == target_dim:
        return x.clone()
...
def _pad_observation(obs: ProtocolObservation, target_dim: int) -> ProtocolObservation:
    padded_value = _pad_tensor_last_dim(obs.value, target_dim)
    meta = dict(obs.metadata or {})
    meta["original_y_dim"] = int(obs.value.numel()) if obs.value.ndim > 0 else 1
    meta["padded_y_dim"] = int(target_dim)
    return replace(obs, value=padded_value, metadata=meta)


def _pad_sample(sample: BenchmarkSample, target_dim: int) -> BenchmarkSample:
    padded_target = _pad_tensor_last_dim(sample.target_value, target_dim)
    padded_obs = tuple(_pad_observation(obs, target_dim) for obs in sample.observations)
    meta = dict(sample.metadata or {})
    meta["original_target_dim"] = int(sample.target_value.numel()) if sample.target_value.ndim > 0 else 1
    meta["padded_target_dim"] = int(target_dim)
    return replace(
        sample,
        target_value=padded_target,
        observations=padded_obs,
        metadata=meta,
    )


def _pad_build_result_targets(
    build_result: Problem1AEllipsoidDatasetBuildResult,
    target_dim: int,
) -> Problem1AEllipsoidDatasetBuildResult:
    padded_samples = tuple(_pad_sample(s, target_dim) for s in build_result.samples)
    meta = dict(build_result.metadata or {})
    meta["common_padded_output_dim"] = int(target_dim)
    return replace(build_result, samples=padded_samples, metadata=meta)


def _build_problem1a_ellipsoid_dataset_from_args(
    args: argparse.Namespace,
) -> Problem1AEllipsoidDatasetBuildResult:
    cfg = Problem1AEllipsoidDatasetBuilderConfig(
        ...
        chart_radius=float(args.chart_radius),
        ellipsoid_center=tuple(float(x) for x in args.ellipsoid_center),
        ellipsoid_scales=tuple(float(x) for x in args.ellipsoid_scales),
        chart_sign=int(args.chart_sign),
    ...
    build_result = build_problem_1a_ellipsoid_dataset(cfg)
    build_result = _pad_build_result_targets(build_result, int(args.common_output_dim))
    return build_result
    ...
    if predictor_type == "mcd":
        def predictor_factory() -> torch.nn.Module:
            return _build_problem1a_mcd_predictor(
                ...
                common_output_dim=int(args.common_output_dim),
                device=device,
            )
...
def _run_active_learning_mode(
    *,
    ...
    runner_cfg = ActiveLearningRunnerConfig(
        ...
        fantasy_train_config=fantasy_cfg if str(args.target_acquisition_policy).lower() == "fantasy" else None,
...
def main() -> None:
    ...
    build_result = _build_problem1a_ellipsoid_dataset_from_args(args)
    ...
    if args.experiment_mode == "fantasy_al":
        ...
        summary = _make_active_learning_summary(
            experiment_name="protocol1a_ellipsoid",
