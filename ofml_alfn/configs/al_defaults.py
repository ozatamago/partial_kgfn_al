...
def _attach_group_defaults(options: Dict, problem=None) -> Dict:
...
def get_default_al_options(problem=None) -> Dict:
    ...
    return _attach_group_defaults(options, problem)


def get_default_sink_only_options(problem=None) -> Dict:
    """
    Preset for sink-only experiments.

    Design intent:
      - no partial node queries
      - acquisition is defined on sink uncertainty by default
      - keep backend / model defaults identical to get_default_al_options()
        unless sink-only behavior requires an override
    """
    ...
    options.update(
        {
            ...
            "selector_objective": "uncertainty",
            "selector_metric": "sink_test_loss",
            # keep fantasy knobs available for future sink-only fantasy extension
            "fantasy_train_steps": 20,
            "fantasy_topk_candidates": 8,
            "fantasy_topk_groups": 2,
        }
