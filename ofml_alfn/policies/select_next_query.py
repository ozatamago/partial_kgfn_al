...
from ofml_alfn.data.update_buffers import append_partial_observation
from ofml_alfn.metrics.evaluation import compute_test_loss
from ofml_alfn.policies.candidates import make_candidates
from ofml_alfn.policies.node_input_builder import (
...
from ofml_alfn.uncertainty.base import (
    predict_mean_var,
    predict_sink_mean_var,
)
from ofml_alfn.uncertainty.mc_dropout import select_top_cost_aware_action
from ofml_alfn.utils.effective_costs import effective_group_costs
...
def _safe_cost_normalize(
    ...
    *,
    ...
    return float(improvement) / max(float(acquisition_cost), float(eps))


def _compute_selector_target_validation_loss(
    *,
    predictor: nn.Module,
    ...
    """
    Compute the target validation loss used by fantasy acquisition.

    Default behavior:
      - interpret selector_holdout_X / selector_holdout_y as the target validation set
      - compute loss with the existing tensor-based evaluator

    Extension hook:
      - if options["target_validation_loss_fn"] is provided and callable,
        it is used instead. This lets you swap in a protocol-aware evaluator later
        without rewriting selector logic.
    """
    custom_loss_fn = options.get("target_validation_loss_fn", None)
...
def _score_candidate_group_by_fantasy_gain(
    *,
    ...
    """
    Return the raw target-validation improvement induced by one fantasy update.

    The returned value is:
        current_target_val_loss - fantasy_target_val_loss

    Cost normalization is handled outside this function so the caller can choose
    the exact acquisition cost corresponding to the selected node group.
    """
    ...
    fantasy_target_val_loss = _compute_selector_target_validation_loss(
        predictor=fantasy_predictor,
        selector_holdout_X=selector_holdout_X,
        selector_holdout_y=selector_holdout_y,
        options=options,
    ...
    return float(current_target_val_loss - fantasy_target_val_loss)
...
def get_suggested_node_and_input(
    ...
    mc_samples = int(options.get("mc_samples", 100))
    n_sobol = int(options.get("cand_n_sobol", 256))
    enable_partial_queries = bool(options.get("enable_partial_queries", False))
    debug_selector = bool(options.get("debug_selector", True))
    ...
    if not enable_partial_queries:
        ...
        if debug_selector:
            logger.info(
                "[selector] mode=full_sink_uq "
                f"max_sink_var={float(pred_var.max().item()):.6f}"
            )
    ...
    use_upstream_first = bool(options.get("use_upstream_first", True))
    ...
    for node_group in node_groups:
        try:
            ...
            # Example: a backend may fail when a node has no usable training data yet.
    ...
    if len(affordable_group_indices) == 0:
        ...
        if debug_selector:
            logger.info(
                "[selector] no affordable partial group; "
    ...
    if selector_objective == "uncertainty":
        ...
        current_target_val_loss = _compute_selector_target_validation_loss(
            predictor=predictor,
            selector_holdout_X=selector_holdout_X,
            selector_holdout_y=selector_holdout_y,
            options=options,
            ...
            for group_idx_candidate in shortlisted_group_indices:
                ...
                for cand_idx_candidate in shortlisted_candidate_indices:
                    ...
                    fantasy_scores[cand_idx_candidate, group_idx_candidate] = (
                        _safe_cost_normalize(
                            raw_improvement,
                            group_cost_candidate,
                        )
                    )

        if torch.isfinite(fantasy_scores).any():
            masked_scores = fantasy_scores
            selection_mode = "partial_group_fantasy_target_val_gain"
        else:
            masked_scores = masked_proxy_scores
    ...
    if debug_selector:
        ...
        logger.info(
            ...
            f"current_target_val_loss="
            f"{'N/A' if current_target_val_loss is None else f'{current_target_val_loss:.6f}'}, "
    ...
    node_candidate = {
        "mode": "partial_group_fantasy_target_val_gain",
        ...
        "current_target_val_loss": (
            None if current_target_val_loss is None else float(current_target_val_loss)
        ),
